...
class silverUpload
{
	...
	*/
	function __construct()
	{
		...
		//返回的json
		// $time = date('Ymd',time());
		$fileAddr = "/Uploads/files/".session('user_id');
		$this->TempFolderPath=$fileAddr;	//文件保存的基准文件夹路径
	...
	{	
		...
		ECHO "serverfilesize:\"".$this->serverfilesize."\",";
		ECHO "info:\"".$this->info."\",";
		ECHO "validatepos:\"".$this->validatepos."\",";
		ECHO "validatelength:\"".$this->validatelength."\",";
		ECHO "streammd5:\"".$this->streammd5."\",";
...
// $silverUpload=new silverUpload();

// $act=$_POST["act"];

// $params=$_POST["params"];		//用户自定义附加参数

// if($act=="validate")
// {
// 	//校验服务器上是否存在相同文件请求
// 	$silverUpload->validate();
// }
// else if($act=="md5")
// {
// 	//文件完整md5校验请求
