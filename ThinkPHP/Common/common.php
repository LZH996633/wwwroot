...
/* function SendMail($address,$title,$message)
{
   ...
   // $mail->SMTPAuth=true;
    // 设置用户名和密码。
    $mail->Username="13721059073@163.com";
    $mail->Password="feng687881";
    // 发送邮件。
...
function call_phpmailer($username, $password, $FromName, $to, $toName, $subject, $body)
{
    require '/ThinkPHP/Extend/Vendor/phpmailer/class.phpmailer.php';
    ...
    $webmaster_email = $username; //回复邮件至此邮箱

    $mail->From = $webmaster_email;
    $mail->AddAddress($to,$toName); //收件人地址和名称
    $mail->AddReplyTo("noreply","请勿回复此邮箱");
    $mail->WordWrap = 30; //每30字断一次行

    //$mail->AddAttachment($file);  //添加附件
    $mail->IsHTML(true);  //发送HTML格式邮件
    $mail->Subject = $subject;  //邮件标题
    $mail->Body = $body;  //邮件内容
    $mail->AltBody = $body;  //纯文字格式的 邮件内容
...
//        return "<span style='font-size:12px;font-weight:bold;text-align:center;color:red;'>邮件发送失败：" . $mail->ErrorInfo . "</span>" ;
        $data['success']=false;
        $data['message']="邮件发送失败：" . $mail->ErrorInfo;
        return  $data;
        //error
    }else{
